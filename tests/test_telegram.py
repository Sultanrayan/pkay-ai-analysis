...
from tradingbot.domain import NewsItem, SentimentSnapshot, Symbol, Timeframe
...
def _build_report() -> AnalysisReport:
    ...
    decision = decision_agent.decide(tech, sent, risk, corr)
...
def test_report_html_contains_sections_and_value():
    ...
    assert report.decision.signal in html
...
def test_welcome_and_history_pages():
    en = I18n("en")
    assert "BTCUSD" in welcome_html(en, "Ada")
    assert "/analyze" in welcome_html(en, None)

    from tradingbot.storage.models import HistoryRow

    rows = [
        HistoryRow(id=1, user_id=1, symbol="BTCUSD", timeframe="1h",
                   timestamp=datetime(2026, 9, 5, tzinfo=timezone.utc),
                   current_price=60000.0, technical_score=10, sentiment_score=0,
                   risk_score=0, correlation_score=0, total_score=10, signal="BUY",
                   summary="s"),
    ]
    text = history_list_html(rows, en, limit=10)
    assert "BTCUSD 1H" in text
...
def test_callback_payloads_roundtrip():
    assert cb.parse(cb.pick_asset(Symbol.BTCUSD)) == ("asset", "BTCUSD")
    assert cb.parse(cb.pick_timeframe("BOTH", Timeframe.H4)) == ("tf", "BOTH", "4h")
